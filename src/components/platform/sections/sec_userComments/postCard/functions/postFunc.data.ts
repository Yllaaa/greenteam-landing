...
export const fetchPosts = async (
  ...
  try {
    ...
    const mainTopicParam = mainTopic === 0 ? "" : `&mainTopicId=${mainTopic}`;

    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_BACKENDAPI}/api/v1/users/commented-posts?limit=${limit}&page=${page}${mainTopicParam}`,
      {
        headers: {
