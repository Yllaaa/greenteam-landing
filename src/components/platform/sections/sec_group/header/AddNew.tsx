...
function AddNew({ onPostComplete }: { onPostComplete?: () => void }) {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [charCount, setCharCount] = useState(0);
  const [isExpanded, setIsExpanded] = useState(false); // New state for collapsible
  const textareaRef = useRef<HTMLTextAreaElement>(null);
  ...
  return (
    <div className={styles.groupForm}>
      <div
        className={`${styles.formHeader} ${styles.clickableHeader}`}
      ...
      <div className={`${styles.contentBar} ${isExpanded ? styles.expanded : styles.collapsed}`}>
        <form className={styles.forumForm} onSubmit={handleSubmit(onSubmit)}>
          {/* Text Area with character count */}
          <div className={styles.textAreaContainer}>
            <textarea
